...
N = 256  # Increase to capture more user preferences
  # Adjust as needed
...
__global__ void matmul(float *A, float *B, int N, int raters, int movies, Row *rows, float learning_rate) {
    ...
    float predicted_rating_pos = 0.0;
    for (int k = 0; k < N; k++) {
        predicted_rating_pos += A[row * N + k] * B[k * raters + col];
    }

    // Sample a negative movie (randomly choose one that the user hasn't rated)
    int negative_movie = (row + blockIdx.x + threadIdx.x) % movies;
    float predicted_rating_neg = 0.0;
    for (int k = 0; k < N; k++) {
        predicted_rating_neg += A[negative_movie * N + k] * B[k * raters + col];
    }

    // Compute BPR loss
    float loss = logf(1 + expf(predicted_rating_neg - predicted_rating_pos));

    // Gradient updates
    for (int k = 0; k < N; k++) {
        float grad_A = loss * (B[k * raters + col]);
        float grad_B = loss * (A[row * N + k]);

        grad_A = fminf(fmaxf(grad_A, -5.0f), 5.0f);
        grad_B = fminf(fmaxf(grad_B, -5.0f), 5.0f);
...
initial_lr = np.float32(0.1)  # Start slightly lower than 0.003
decay_factor = np.float32(0.9)   # Reduce by 10% every 5 epochs
decay_step = 5
...
for epoch in range(epochs):
    learning_rate = np.float32(max(initial_lr * (decay_factor ** (epoch // decay_step)), min_lr))
    ...
    cuda.memcpy_dtoh(matrixBUsers_result, B_gpu_matrixBUsers)
    cuda.memcpy_dtoh(rows_result, rows_gpu)
    matrixAMovies_result = normalize_embeddings(matrixAMovies_result)
    matrixBUsers_result = normalize_embeddings(matrixBUsers_result)
